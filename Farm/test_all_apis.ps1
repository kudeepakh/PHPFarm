...
# Login
$loginResp = Invoke-WebRequest -Uri "$baseUrl/api/v1/auth/login" -Method POST -Headers @{
    "Content-Type" = "application/json"
    "X-Correlation-Id" = "login"
    "X-Transaction-Id" = "login"
    "X-Request-Id" = "login"
} -Body '{"email":"test@example.com","password":"Test@1234"}' -UseBasicParsing
...
foreach ($endpoint in $endpoints) {
    try {
        $resp = Invoke-WebRequest -Uri "$baseUrl$($endpoint.Path)" -Method GET -Headers @{
